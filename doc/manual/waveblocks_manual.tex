...
the trunk) into a new directory (here \verb|~/WBT|)

\begin{verbatim}
  svn co https://svn.origo.ethz.ch/waveblocks/trunk ~/WBT
\end{verbatim}
...
\begin{verbatim}
  mkdir ~/python
  cp -r ~/WBT/src/numdifftools/ ~/python
\end{verbatim}

We can just copy the files as they won't change in the svn repository
often. But it's a wise idea not to copy \verb|~/WBT/src/WaveBlocks| but
just use symbolic links. (If you have multiple checkouts this is also
a way to choose which one will be used when you just type \texttt{import WaveBlocks}
at a python prompt.) You may just follow the shell commands given
here for a working sample setup. First we create the necessary
directory structure and the we soft-link the files.

\begin{verbatim}
  mkdir ~/python/WaveBlocks
  mkdir ~/python/WaveBlocks/Plot
  ln -s ~/WBT/src/WaveBlocks/*.py ~/python/WaveBlocks/
  ln -s ~/WBT/src/WaveBlocks/Plot/*.py ~/python/WaveBlocks/Plot/
\end{verbatim}
