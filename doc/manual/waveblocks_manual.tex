...
\begin{description}
  ...
  \item[\texttt{spawn\_normed\_gaussian}] Specify the spawning algorithm used.
  If \texttt{True} we spawn just a normed Gaussian packet by copying over the norm
  of the \emph{spawn candidate}. If \texttt{False} a full basis projection is done
  up to the maximal order given by the parameter \texttt{spawn\_max\_order}.
  \begin{itemize}
    \item Possible values: True or False
...
See the IOM API and the IOM plugins.
