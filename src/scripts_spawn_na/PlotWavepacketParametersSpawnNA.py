...
def plot_parameters_spawn(timegrids, AllPA):
    ...
    for index in xrange(len(timegrids)):
        ...
        ax[0].plot(grid, real(Phist), "-o", label=r"$\Re P$")
        ax[0].grid(True)
        ax[0].set_title(r"$\Re P$")
    
        ax[1].plot(grid, imag(Phist), "-o", label=r"$\Im P$")
        ax[1].grid(True)
        ax[1].set_title(r"$\Im P$")
    
        ax[2].plot(grid, real(Qhist), "-o", label=r"$\Re Q$")
        ax[2].grid(True)
        ax[2].set_title(r"$\Re Q$")
    
        ax[3].plot(grid, imag(Qhist), "-o", label=r"$\Im Q$")
        ax[3].grid(True)
        ax[3].set_title(r"$\Im Q$")
    
        ax[4].plot(grid, real(qhist), "-o", label=r"$q$")
        ax[4].grid(True)
        ax[4].set_title(r"$q$")
    
        ax[5].plot(grid, real(phist), "-o", label=r"$p$")
        ax[5].grid(True)
        ax[5].set_title(r"$p$")
    
        ax[6].plot(grid, real(Shist), "-o", label=r"$S$")
        ax[6].grid(True)
        ax[6].set_title(r"$S$")
