...
are some methods for calculating data as for example $L^2$
...
class WaveFunction:
    """This class represents a vector valued quantum state $\Ket{\Psi}$ as used
    in the vector valued time-dependent Schroedinger equation. The state
    $\Ket{\Psi}$ is composed of $\psi_0, \ldots, \psi_{N-1}$ where $\psi_i$ is
    ...
    def get_norm(self, values=None, summed=False, component=None):
        """Calculate the $L^2$ norm of the whole vector $\Ket{\Psi}$ or some
        individual components $\psi_i$. The calculation is done in momentum space.
