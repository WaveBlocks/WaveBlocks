...
class SimulationLoopHagedorn(SimulationLoop):
    ...
    def prepare_simulation(self):
        ...
        self.IOManager.save_coefficients(self.propagator.get_wavepacket().get_coefficients(), timestep=0)
        self.IOManager.save_parameters(self.propagator.get_wavepacket().get_parameters(), timestep=0)
    ...
    def run_simulation(self):
        ...
        for i in xrange(1, self.nsteps+1):
            ...
            if tm.must_save(i):
                self.IOManager.save_coefficients(self.propagator.get_wavepacket().get_coefficients(), timestep=i)
                self.IOManager.save_parameters(self.propagator.get_wavepacket().get_parameters(), timestep=i)
