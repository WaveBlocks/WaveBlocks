...
def color_map(data, phase=None, modulus=None, darken=False, rescale=False):
    ...
    if rescale is True:
        # The factor 0.75 is arbitrary but smaller than 1 such that we don't loose
        # phase information by getting a pure white color for maximal entries.
        hsv_colors[...,2] = hsv_colors[...,2] / (0.75*hsv_colors[...,2].max())
        # Adjust saturation
        l = hsv_colors[...,2]
        hsv_colors[...,1] = where(l <= 0.5, 2*l, 2*(1-l))
